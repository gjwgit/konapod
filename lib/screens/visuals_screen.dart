...
class VisualsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final best      = stats.map((d) => d.distanceKm)
        .reduce((a, b) => a > b ? a : b);
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        // ── Summary cards ─────────────────────────────────────────────
        _SummaryRow(total: total, avg: avg, best: best,
            days: stats.length),
        ...
        // ── Detail table ──────────────────────────────────────────────
        _DetailTable(stats: stats, hasEnergy: hasEnergy,
            hasRegen: hasRegen),
        const SizedBox(height: 24),
      ]),
    );
  ...
  Widget _placeholder(BuildContext context, String title, String sub) =>
      Center(
        child: Padding(
...
class _SummaryRow extends StatelessWidget {
  ...
  final int days;
  const _SummaryRow({required this.total, required this.avg,
      required this.best, required this.days});
  @override
  Widget build(BuildContext context) {
...
class _ChartSection extends StatelessWidget {
  ...
  final Widget chart;
  const _ChartSection({required this.title, required this.subtitle,
      required this.chart});
  @override
...
class _DetailTable extends StatelessWidget {
  ...
  const _DetailTable({required this.stats,
      required this.hasEnergy, required this.hasRegen});
        ...
        ...stats.reversed.map((d) => Column(children: [
          Padding(
            ...
            child: Row(children: [
              ...
              if (hasEnergy) SizedBox(width: 80, child: Text(
                  '${((d.totalConsumed ?? 0) / 1000).toStringAsFixed(2)} kWh',
                  textAlign: TextAlign.right,
                  style: TextStyle(
                      color: cs.onSurface, fontSize: 12))),
              if (hasRegen) SizedBox(width: 72, child: Text(
...
class _Head extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Expanded(child: Text(text,
      textAlign: align,
